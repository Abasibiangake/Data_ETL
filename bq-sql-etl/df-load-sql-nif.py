...
with open('sql_table_names_nif.txt', 'r') as file:
    ...
    for table_name in table_names:
        ...
        bucket_name = f'query-bucket-nif'
        ...
        blob.upload_from_string(f"SELECT *, {today_str} AS LAST_DATE FROM NIF.dbo.{table_name}")

        # Get the GCS URI of the uploaded file
        gcs_uri = f'gs://{bucket_name}/{query_filename}'

        command_template = 'gcloud dataflow jobs run job_dtf_nif_AOL_CHATBOT_CREDENCIALES ' \
        ...
        connection_url = 'jdbc:sqlserver://172.35.30.7:1433;databaseName=NIF;encrypt=false'
        driver_class_name = 'com.microsoft.sqlserver.jdbc.SQLServerDriver'
        query_file = gcs_uri
        output_table = f'tst-peinnovabi-data-storage:ds_nif_raw_level.nif_{table_name}'  # Replace gen_sede with the table name
