...
def get_yesterday():
    ...
    print('this is yesterdays date: ', yesterday)
    return yesterday.strftime('%Y-%m-%d')

with open('sql_table_names_pea copy.txt', 'r') as file:
    ...
    for table_name in table_names:
        ...
        bucket_name = f'query-bucket-pea'
        ...
        if table_name == 'evaluation':
            # If the table name is 'pea_evaluation', perform a different query
            yesterday_date = get_yesterday()
            blob.upload_from_string(f"SELECT *, {today_str} AS LAST_DATE FROM [PEA_TST].dbo.{table_name} WHERE [Date] = '{yesterday_date}'")
        elif table_name == 'attendanceSynchronousdetail':
            # If the table name is 'pea_evaluation', perform a different query
            yesterday_date = get_yesterday()
            blob.upload_from_string(f"SELECT *, {today_str} AS LAST_DATE FROM [PEA_TST].dbo.{table_name} WHERE [UpdatedDate] = '{yesterday_date}'")
        else: 
            # Upload the query string to the blob
            blob.upload_from_string(f"SELECT *, {today_str} AS LAST_DATE FROM [PEA_TST].dbo.{table_name}")

        # Get the GCS URI of the uploaded file
        gcs_uri = f'gs://{bucket_name}/{query_filename}'
        print("i have gotten the query")
        command_template = 'gcloud dataflow jobs run job_dtf_pea_AOL_CHATBOT_CREDENCIALES ' \
        ...
        query_file = gcs_uri
        output_table = f'tst-peinnovabi-data-storage:ds_pea_raw_level.pea_{table_name}'  # Replace gen_sede with the table name
